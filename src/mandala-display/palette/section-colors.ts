...
export type SectionColorContext = {
    backgroundMode: string;
    sectionColorsBySection: Record<string, string>;
    sectionColorOpacity: number;
};

export type ResolveSectionSurfaceColorOptions = SectionColorContext & {
    section: string | null;
};
...
export const resolveSectionAccentColor = ({
    section,
    sectionColorsBySection,
}: Pick<ResolveSectionSurfaceColorOptions, 'section' | 'sectionColorsBySection'>) => {
    if (!section) return null;
    return sectionColorsBySection[section] ?? null;
};

export const resolveSectionSurfaceColor = ({
    section,
    backgroundMode,
    sectionColorsBySection,
    sectionColorOpacity,
}: ResolveSectionSurfaceColorOptions) => {
...
export const resolveGrayBlockSurfaceColor = (sectionColorOpacity: number) =>
    `color-mix(in srgb, var(--mandala-gray-block-base) ${sectionColorOpacity}%, transparent)`;

export const resolveSectionBackgroundInput = (
    options: ResolveSectionSurfaceColorOptions,
) => resolveSectionSurfaceColor(options);
